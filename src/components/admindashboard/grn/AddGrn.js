...
const AddGrn = ( { onSave, onClose } ) => {
  ...
  const [columns, setColumns] = useState([
    { key: "material", label: "Material/Article Name" },
  ...
  const handleTableDataChange = (rowIndex, colKey, value) => {
    setTableData((prev) => {
      ...
      if (colKey === "quantity" || colKey === "receivedQuantity") {
        newData[rowIndex].shortage = calculateShortage(newData[rowIndex]);
      }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-[linear-gradient(346.16deg,#2B86AA_42.92%,#69B9D9_139.13%)] border border-[#ABE7FF] text-white px-6 py-4 rounded-lg shadow-lg">
          <div className="flex items-center justify-between mb-3 pb-3 border-b border-white/30">
            <h1 className="text-md font-regular">Goods Received Note</h1>
        ...
        <div className="border border-[#B3E2FF] mt-5 rounded-lg">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 py-2 px-5 gap-4">
            {["pONo", "pODate", "department", "plantCode", "reference"].map(
              (field) => (
        ...
        {/* vender Details Header */}
        <div className="border border-[#B3E2FF] mt-5 rounded-lg">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 py-2 px-5 gap-4">
            {["venderCode", "venderName", "address", "contactPerson"].map(
              (field) => (
                <div key={field} className="flex flex-col">
                  <label className="text-xs mb-1 h-6 flex items-center capitalize">
                    {formatFieldLabel(field)}
                  ...
                  {field === "venderCode" ? (
                    <div className="relative">
                      ...
                      >
                        <span>
                          {vernderDetails[field] || "Select Vendor Code"}
                        </span>
                        <ChevronDown size={12} />
                      </button>
                      {vendorDropdownOpen && (
            ...
            <table className="w-full border-collapse table-auto">
              <thead>
                <tr className="bg-white border-b-2 border-blue-200">
                  ...
                  <th className="w-8 px-0 py-1 text-xs font-medium text-gray-700 text-center border-blue-200 bg-white"></th>
                </tr>
              </thead>
              <tbody>
                    ...
                    {columns.map((col, colIndex) => (
                      ...
                      >
                        {col.key === "plantCode" ? (
                              ...
                              poDetails.plantCode && (
                                <div className="absolute z-50 left-0 mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-32 overflow-y-auto min-w-32">
                                  {plantCodes
                                    .filter(
                                      (plant) =>
                                        plant.code === poDetails.plantCode
                                    )
                                    .map((plant) => (
                                      <div
                                        key={plant.code}
                          ...
                          <input
                            type={
                              // Text fields: material and uom
                              ["material", "uom"].includes(col.key)
                                ? "text"
                                : "number" // Number fields: quantity, receivedQuantity, shortage, damage
                            }
                            value={row[col.key] || ""}
                            onChange={(e) =>
                              handleTableDataChange(
        ...
        <div className="flex justify-center space-x-4 my-6">
          <button className="bg-[#2B86AA] text-white px-8 py-2 rounded-md shadow-sm hover:bg-[#227499] transition-colors"  type="submit"
          onClick={() => {
            const newGrnData = { /* your form data */ };
            onSave(newGrnData);
          }}>
            Publish
          </button>
          <button className="bg-white border border-gray-300 text-gray-700 px-8 py-2 rounded-md shadow-sm transition-colors " onClick={onClose}>
            Cancel
          </button>
        </div>
      </div>
